...
# --- FIX 1: Set Secret BEFORE importing server to prevent crash ---
os.environ["JWT_SECRET"] = "testing_secret_key_1234567890_min_32_chars"

from server import app, generate_rsa_keypair, encrypt_file_pure_rsa, decrypt_file_pure_rsa, users_db
...
class TestSecureRSAService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.alice_token = self.get_auth_token("alice", "Alice@Secret1!") # Decrypt only
        self.bob_token   = self.get_auth_token("bob", "Bob@Secret2!")     # Encrypt only
    ...
    # -------------------------------------------------------------------------
    # 1. UNIT TESTS (Pure Math Logic)
    # -------------------------------------------------------------------------
    def test_rsa_math_roundtrip(self):
        """Verify the pure RSA chunking logic works without the API."""
        priv, pub = generate_rsa_keypair(2048)
        original_data = b"Start" + os.urandom(300) + b"End" # > 256 bytes to force chunking
    ...
    def test_rbac_segregation(self):
        ...
        # Bob Encrypts -> OK (He is 'encrypt_user')
        res = self.app.post(f'/api/v1/files/{file_id}/encrypt', 
                            headers={'Authorization': f'Bearer {self.bob_token}'})
        self.assertEqual(res.status_code, 200)

        # Bob Decrypts -> FORBIDDEN (He is NOT 'decrypt_user')
        res = self.app.post(f'/api/v1/files/{file_id}/decrypt', 
                            headers={'Authorization': f'Bearer {self.bob_token}'})
        self.assertEqual(res.status_code, 403) # <--- Security Check
    ...
    def test_idor_protection(self):
        """Verify Admin cannot see Bob's files (Owner Isolation)."""
        # Bob Uploads
        res = self.app.post('/api/v1/files/upload', 
                            headers={'Authorization': f'Bearer {self.bob_token}'},
        ...
        # Admin tries to Decrypt Bob's file -> FORBIDDEN (Not Owner)
        # Even though Admin has the 'decrypt' role, they don't own the file.
        res = self.app.post(f'/api/v1/files/{bobs_file_id}/decrypt', 
                            headers={'Authorization': f'Bearer {self.admin_token}'})
        self.assertEqual(res.status_code, 403) # <--- IDOR Check
